...
class Input {
	...
	public:
		...
		static Input* Construct();


		static uint64_t InputBind(std::uint32_t Type, fastdelegate::FastDelegate0<> Delegate);
		static uint64_t InputBind(SDL_Keycode Key, fastdelegate::FastDelegate1<Uint8> Delegate);
		static uint64_t InputBind(std::uint32_t Type, Uint8 WindowAction, fastdelegate::FastDelegate0<> Delegate);

		static void InputUnbind(const uint64_t uniqueID);
